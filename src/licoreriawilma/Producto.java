...
//              regs[c][2]=res.getString("fechaAgregado");
    ...
    public Object[][] listaSearch(boolean desactivar,String nomProS, int numero){
        ...
        switch(numero){
            case 1:
                //En este caso seleccionaremos por nombre de Tipo Producto
                ...
                break;
            case 2:
                //En este caso seleccionaremos por nombre de Producto
                sqlSearch =  "SELECT count(*) " +
                             "FROM Producto " +
                             "INNER JOIN TipoProducto " +
                             "on Producto.idTipoProducto = TipoProducto.idTipoProducto " +
        ...
        try {
            PreparedStatement pstm = conn.getConnection().prepareStatement(sqlSearch);
            ResultSet res = pstm.executeQuery();
            if(res.next())
                total = res.getInt(1);
            //conn.Desconectar();
        } catch (SQLException e) {
            System.out.println("Total selectSearch:");
            System.out.println(e);
        }
        
        
        Object[][] obj = new Object[total][5];
        ...
        try {
            ...
            while(res.next()){
                obj[c][0] = c+1;
                obj[c][1] = res.getString("nombreProducto");
                obj[c][2] = res.getString("nombreTipoProducto");
                obj[c][3] = res.getInt("stock");
                obj[c][4] = res.getDouble("precio");
                c++;
            }
